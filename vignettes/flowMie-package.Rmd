...
`flowMie` provides tools to describe small particles, including calibration beads as well as EVs, along with an optical description of the flow cytometer laser and detector systems, that enable a complete and accurate modeling environment to calculate and characterize the response of your flow cytometer to these very small and dim particles.  We also introduce a new concept - **the Mie Transform** - which uses calibrated flow cytometry data to directly compute the diameter of each event in an FCS file.
...
detector = create_detector(theta_0 = 90, alpha = 60, psi_0 = 90, pol = 1.0)
```
...
ffs = mie_transform(ff = ff_ev, ssc_param = "SSC-H", transformation = 'biexp', lut = calibrate$lut)
...
For the first time, we are able to directly visualize quantitative size distributions from flow cytometry data.
